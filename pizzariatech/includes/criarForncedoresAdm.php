...
if (isset($_POST['idFornecedores']) && !empty($_POST['idFornecedores'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        header("Location: ../FornecedoresAdm.php?tipo=sucesso&mensagem=Fornecedor atualizada com sucesso");
    } else {
        header("Location: ../FornecedoresAdm.php?tipo=erro&mensagem=Erro ao atualizar Fornecedor");
    }
} else {
    // Inserir
    ...
    if (mysqli_query($conn, $sql)) {
        header("Location: ../FornecedoresAdm.php?tipo=sucesso&mensagem=Fornecedor cadastrada com sucesso");
    } else {
        header("Location: ../FornecedoresAdm.php?tipo=erro&mensagem=Erro ao cadastrar o fornecedor");
    }
}
?>
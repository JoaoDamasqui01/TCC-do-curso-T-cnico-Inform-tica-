<?php
    ...
    if($idPedidosItens == 0){
			...
			if (mysqli_query($conn, $sql)) {
					header("Location:../PedidosFinalAdm.php?tipo=sucesso&mensagem=Seu Pedido Final realizado com sucesso");
				} else {
        ...
        $sql = "UPDATE pedidositens SET idItens = '$idItens', idPedidos = '$idPedidos', idEntregas = '$idEntregas', qtde = '$qtde', precoUnitario = '$precoUnitario', precoTotal = '$precoTotal' WHERE idPedidosItens = '$idPedidosItens'";
			if (mysqli_query($conn, $sql)) {
				header("Location: ../PedidosFinalAdm.php?tipo=sucesso&mensagem=O cadastro do Pedido Final foi atualizado com sucesso");
			}
			else{
				header("Location: ../PedidosFinalAdm.php?tipo=erro&mensagem=ERRO-cadastro do Pedido Final não foi atualizado");
			}
    }
